...
export function Mechanism({ compact = false }: { compact?: boolean }) {
  return (
    <section className={`mechanism ${compact ? "mechanism--compact" : ""}`} aria-labelledby={compact ? undefined : "mechanism-title"}>
      ...
      {!compact && <div className="mechanism-action"><Link className="button button--yellow" href="/data-wellness">Explore data wellness →</Link></div>}
    </section>
  );
}

export function CtaBand({ eyebrow, title, text, href, label }: { eyebrow: string; title: string; text: string; href: string; label: string }) {
  const link = href.startsWith("mailto:") ? <a className="button button--yellow" href={href}>{label} ↗</a> : <Link className="button button--yellow" href={href}>{label} →</Link>;
...
export function PageHero({ eyebrow, title, deck, primary, secondary }: {
  ...
  const link = (action: { label: string; href: string }, className: string) => action.href.startsWith("mailto:") ? <a className={className} href={action.href}>{action.label} ↗</a> : <Link className={className} href={action.href}>{action.label} →</Link>;
