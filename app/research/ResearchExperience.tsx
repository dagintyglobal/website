...
const fields: Array<{
  ...
  { key: "headcount", label: "Headcount", min: 50, max: 500, step: 1, hint: "50-500 employees", tooltip: "Number of employees covered by the program. Platform cost is calculated at $160 per employee annually.", format: (v) => Math.round(v).toLocaleString("en-US") },
  { key: "revenue", label: "Annual revenue", min: 5_000_000, max: 100_000_000, step: 500_000, hint: "$5M-$100M", tooltip: "The organization’s annual revenue, used with the enterprise value multiple to estimate enterprise value.", format: (v) => fmtUSD(v) },
  { key: "evmultiple", label: "Enterprise value multiple", min: 0.5, max: 5, step: 0.1, hint: "0.5x-5x revenue", tooltip: "The revenue multiple used to estimate enterprise value based on the organization’s industry and financial profile. This is the value potentially lost due to bankruptcy when a breach is involved.", format: (v) => `${v.toFixed(1)}×` },
