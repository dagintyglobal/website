    ...
    <main>
      <section className="home-hero">
        <div className="home-hero-copy">
          ...
          <div className="hero-actions"><a className="button button--yellow" href="/data-wellness">Explore data wellness →</a><a className="text-link text-link--light" href="/ecosystem">Explore the ecosystem →</a></div>
        </div>
      ...
      <section className="role-section">
        ...
        <div className="role-section-copy">
          ...
          <div className="role-grid">{roles.map(role => <a href={role.href} className="role-card" key={role.number}><span>{role.number}</span><h3>{role.title}</h3><p>{role.text}</p><b>{role.action} →</b></a>)}</div>
        </div>
      </section>
      ...
      <section className="now-next-section">
        ...
        <div className="now-next-grid">
          <article className="now-card"><span>01</span><h3>Data wellness</h3><p>Build practical habits that help people understand, protect, and take control of their personal data.</p><a href="/data-wellness">Explore wellness →</a></article>
          <article className="now-card"><span>02</span><h3>Threat reduction</h3><p>Use personal data wellness to give employers and their trusted partners a measurable way to lower human-driven risk.</p><a href="/products">Explore products →</a></article>
          <article className="platform-card"><span>03</span><h3>Data infrastructure</h3><p>Give data buyers trusted personal data backed by consent, provenance, accountability, and a receipt for every data point.</p><a href="/ecosystem">Explore model →</a></article>
        </div>
      </section>

      <section className="roi-teaser">
        <div className="roi-whitepaper"><Eyebrow>Research Lab</Eyebrow><h2>Employee data wellness as cybersecurity.</h2><div className="roi-whitepaper-thesis"><strong>Attackers do not need to breach your firewall if they can breach your people.</strong><p>Data brokers package the personal details that make phishing credible: family relationships, addresses, phone numbers, work history, purchases, and habits. Traditional annual training asks employees to resist the result. Cluzy reduces the raw material.</p></div><p>Read the evidence and risk-reduction framework behind Dāginty. The whitepaper explains how personal-data exposure makes social engineering work, how data wellness reduces that exposure, and how progress can be measured.</p><div className="roi-half-action"><a className="button button--dark" href="/research?view=whitepaper">Read the whitepaper →</a></div></div>
        <div className="roi-calculator"><CalculatorChartPreview/><div className="roi-half-action"><a className="button button--dark" href="/research?view=calculator">Open the calculator →</a></div></div>
      </section>
    </main>
  </PageShell>;
}
